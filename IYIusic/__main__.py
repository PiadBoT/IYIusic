...
import config
from IYIusic import LOGGER, app, userbot
from IYIusic.core.call import AMBOT
from IYIusic.misc import sudo
from IYIusic.plugins import ALL_MODULES
from IYIusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("IYIusic.plugins" + all_module)
    LOGGER("IYIusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await AMBOT.start()
    try:
        await AMBOT.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("IYIusic").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("IYIusic").info("Stopping AnieXErica  Music Bot...")
